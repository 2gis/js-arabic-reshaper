...
// Конвертирует формы символов в компактное представление в виде массива.
// ВНИМАНИЕ: Индексы в этом массиве должны соотвествовать константам в index.js
function convertForms (formsObj) {
    return [
        formsObj.normal || 0,
        formsObj.isolated || 0,
        formsObj.initial || 0,
...
function insertTrie(trie, key, payload) {
    let curr = trie;
    for (let char of key) {
        const charCode = char.charCodeAt(0);
        if (!curr[charCode]) {
            curr[charCode] = makeTrie();
        }
        curr = curr[charCode];
    }
    if (curr[VAL_INDEX]) {
        console.error(`Conflicting values on key=${key}. Existing value: ${JSON.stringify(curr[VAL_INDEX])} new value ${JSON.stringify(payload)}`)
        return;
    }
    curr[VAL_INDEX] = payload;
}
...
const LIGATURES_TRIE = makeTrie();
for (const liga of LIGATURES) {
    for (const m of liga.matches) {
        insertTrie(LIGATURES_TRIE, m, convertForms(liga.forms));
    }
}
fs.writeFileSync('ligas.json', JSON.stringify(LIGATURES_TRIE));
...
for (const letter in LETTERS) {
    LETTERS[letter] = convertForms(LETTERS[letter]);
}
